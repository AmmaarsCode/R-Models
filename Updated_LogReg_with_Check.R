...
bank.df <- read.csv("C:\\RImport\\UniversalBank.csv")

# Check if 'Personal.Loan' is in the dataset
if (!("Personal.Loan" %in% names(bank.df))) {
    stop("Error: 'Personal.Loan' variable not found in the dataset.")
}
...
glm(Personal.Loan ~ Age + predictors, family = "binomial")
# regression.
...
# Generating histograms for actual and predicted values
hist(forplots$actual, main="Actual Values", xlab="Value", col="blue")
hist(forplots$predicted, main="Predicted Values", xlab="Value", col="red")
